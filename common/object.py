...
# get_rotation(reverse) rotation of object in degrees
...
class ObjectData:
    ...
    def get_rotation(self, reverse=False):
        dy = -self.dy if reverse else self.dy
        rads = math.atan2(dy, self.dx)
        rads %= 2*math.pi
        return int(round(math.degrees(rads)))
